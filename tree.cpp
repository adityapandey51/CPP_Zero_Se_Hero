...
// in this sol if all the elements in the tree are unique then it works;
//because the index of duplicate element gets updated by the last index where it occured
// tc:O(nlogn)
class Solution{
    private:
    ...
    Node* constructTree(int in[], int pre[], int inOrderStart, int inOrderEnd, int &index, map<int,int> &nodeToIndex, int size){
    ...
    Node* buildTree(int in[],int pre[], int n)
    {
        ...
        Node* ans = constructTree(in, pre, 0, n-1, index, nodeToIndex, n);
        return ans;
    }
};
...
class Solution{
    private:
    int position(int in[], int inOrderStart, int inOrderEnd, int element ){
    ...
    Node* constructTree(int in[], int pre[], int inOrderStart, int inOrderEnd, int &index, int size){
        if(index >= size || inOrderStart > inOrderEnd){
            return NULL;
        }
        int temp = pre[index++];
        Node* root = new Node(temp);
        int pos = position(in, inOrderStart, inOrderEnd, temp);
        
        root->left = constructTree(in, pre, inOrderStart, pos-1, index, size);
        root->right = constructTree(in, pre, pos+1, inOrderEnd, index, size);
    ...
    Node* buildTree(int in[],int pre[], int n)
    {
        ...
        Node* ans = constructTree(in, pre, 0, n-1, index, n);
        return ans;
    }
};
----------------------------------------------------------------------------------------------------
...
Node* constructTree(int in[], int post[], int inOrderStart, int inOrderEnd, int &index, int size){
        if(index >= size || inOrderStart > inOrderEnd){
            return NULL;
        }
        int temp = post[index--];
        ...
        root->right = constructTree(in, post, pos+1, inOrderEnd, index, size);

        root->left = constructTree(in, post, inOrderStart, pos-1, index, size);
...
Node *buildTree(int in[], int post[], int n) {
       int index = n-1;
        ...
        Node* ans = constructTree(in, post, 0, n-1, index, n);
        return ans;
}
-------------------------------------------------------------------------------------------------------
BURNING TREE
